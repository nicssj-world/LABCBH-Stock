-- 20260820020000_pr_manual_catalog_items_fixes.sql added an up-front check for
-- fiscalYear, but wrote it as `p_request ->> 'fiscalYear' !~ pattern` without
-- coalescing first. SQL's three-valued logic makes `null !~ pattern` evaluate
-- to null, not true, and plpgsql's `if null then` is silently false — so a
-- request that omits the field outright (as opposed to sending it blank)
-- skipped this check entirely and fell through to the raw not-null violation
-- on purchase_requests.fiscal_year the check exists to avoid. Caught live
-- against staging: a request with items but no fiscalYear reached the
-- unrelated "must have at least one line" error only because that item-count
-- check happened to run before the fiscal-year cast that would have failed
-- ugly. The contractId check just below already gets this right with
-- coalesce(..., ''); this brings fiscalYear in line with it.
begin;
