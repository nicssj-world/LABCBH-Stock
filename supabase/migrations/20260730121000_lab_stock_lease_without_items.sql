-- Equipment leases hold no line items, but create_contract and update_contract
-- both refused an empty array, so a lease could not be created or edited at all.
-- The requirement becomes conditional on contract type, and the inverse is
-- enforced too: a lease that is handed items is rejected rather than silently
-- accumulating rows the mode guard would later block.
--
-- Both functions are reproduced verbatim from
-- 20260729173550_atomic_contract_creation.sql apart from those guards.
begin;
