...
-- Legacy portal rows were classified during Task 2 without inventing missing
-- lifecycle facts. Keep only those provenance-marked rows editable/archiveable.
...
alter table public.contracts
  ...
  check (
    ...
    or source_metadata ->> 'migration' = 'lab_stock_contract_expansion'
    or (
...
alter table public.contracts
  ...
  check (
    procurement_stage is null
    or (
      source_metadata ->> 'migration' = 'lab_stock_contract_expansion'
      and procurement_stage = 'contract_started'
...
create or replace function public.assert_contract_editor_actor(p_actor_id uuid)
returns void
language plpgsql
security invoker
set search_path = ''
...
begin
  if not exists (
    ...
    raise exception using
      errcode = '42501',
      message = 'actor is not allowed to edit contracts';
  end if;
end
$function$;

revoke execute on function public.assert_contract_editor_actor(uuid) from public;
revoke execute on function public.assert_contract_editor_actor(uuid) from anon;
revoke execute on function public.assert_contract_editor_actor(uuid) from authenticated;
grant execute on function public.assert_contract_editor_actor(uuid) to service_role;
...
begin
  ...
  if exists (
    ...
    where field_name not in ('fiscalYear', 'contractType', 'displayName', 'vendor', 'endDate')
  ) then
    raise exception using errcode = '22023', message = 'unexpected contract field';
  end if;

  if not (p_contract ?& array['fiscalYear', 'contractType', 'displayName', 'vendor', 'endDate'])
...
revoke execute on function public.create_contract(uuid, jsonb, jsonb, date) from public;
revoke execute on function public.create_contract(uuid, jsonb, jsonb, date) from anon;
revoke execute on function public.create_contract(uuid, jsonb, jsonb, date) from authenticated;
...
revoke execute on function public.update_contract(bigint, uuid, jsonb, jsonb, timestamptz) from public;
revoke execute on function public.update_contract(bigint, uuid, jsonb, jsonb, timestamptz) from anon;
revoke execute on function public.update_contract(bigint, uuid, jsonb, jsonb, timestamptz) from authenticated;
grant execute on function public.update_contract(bigint, uuid, jsonb, jsonb, timestamptz) to service_role;

create or replace function public.archive_contract(
  p_contract_id bigint,
  p_actor_id uuid,
  p_reason text
)
...
begin
  ...
  if coalesce(current_contract.is_archived, false) then
    raise exception using errcode = '55000', message = 'contract is already archived';
  end if;
...
revoke execute on function public.archive_contract(bigint, uuid, text) from public;
revoke execute on function public.archive_contract(bigint, uuid, text) from anon;
revoke execute on function public.archive_contract(bigint, uuid, text) from authenticated;
grant execute on function public.archive_contract(bigint, uuid, text) to service_role;
