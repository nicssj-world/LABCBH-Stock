-- Keep plan-reference validation aligned with the indexed PDF text. OCR can
-- insert spaces around Thai marks, decompose sara am differently, or make a
-- small character substitution. A manually confirmed row still has to be
-- recognisably related to the submitted name.
create extension if not exists fuzzystrmatch with schema extensions;

create or replace function public.annual_plan_compact_text(p_value text)
returns text
language sql
immutable
parallel safe
  ...
  select regexp_replace(
    lower(
      replace(
        replace(
          replace(
            replace(coalesce(p_value, ''), chr(8203), ''),
            chr(8204),
            ''
          ),
          chr(8205),
...
create or replace function public.annual_plan_text_is_related(
  p_expected text,
  p_item_name text,
  p_raw_text text
)
...
as $function$
  with normalized as (
    select
      public.annual_plan_compact_text(
        replace(coalesce(p_expected, ''), chr(3659), '')
      ) as expected_text,
      public.annual_plan_compact_text(
        replace(coalesce(p_item_name, ''), chr(3659), '')
      ) as item_text,
      public.annual_plan_compact_text(
        replace(coalesce(p_raw_text, ''), chr(3659), '')
      ) as raw_text,
      public.annual_plan_compact_text(
        replace(
          regexp_replace(
            coalesce(p_item_name, ''),
            '[[:space:]]*[0-9๐-๙][0-9๐-๙,[:space:]]*([.][0-9๐-๙]+)?[[:space:]]*$',
            '',
            'g'
          ),
          chr(3659),
      ...
      ) as raw_text_without_amount
  )
  select normalized.expected_text <> ''
    and exists (
      select 1
      from unnest(array[
        normalized.item_text,
        normalized.raw_text,
        normalized.item_name_without_amount,
        normalized.raw_text_without_amount
      ]) as candidate(value)
      where candidate.value <> ''
        and (
          position(normalized.expected_text in candidate.value) > 0
          or position(candidate.value in normalized.expected_text) > 0
          or (
            least(
              length(normalized.expected_text),
...
-- These helpers are implementation details of the service-role mutation and
-- must not become public RPC surface area.
revoke all on function public.annual_plan_compact_text(text) from public, anon, authenticated;
revoke all on function public.annual_plan_text_is_related(text, text, text) from public, anon, authenticated;
grant execute on function public.annual_plan_compact_text(text) to service_role;
grant execute on function public.annual_plan_text_is_related(text, text, text) to service_role;

-- Recreate the existing function so both the Server Action and the database
-- apply exactly the same OCR-tolerant relationship rule. This is a forward
-- migration; the original migration remains unchanged for existing installs.
...
begin
  ...
  if expected_plan_type = 'hiring' then
    ...
    if not public.annual_plan_text_is_related(contract_name, plan_row.item_name, plan_row.raw_text) then
  ...
  if expected_plan_type = 'procurement' then
    ...
    loop
      ...
      if match_method = 'name_exact'
         and not public.annual_plan_text_is_related(actual_item_name, plan_row.item_name, plan_row.raw_text) then
...
revoke execute on function public.annual_plan_compact_text(text) from public, anon, authenticated;
revoke execute on function public.annual_plan_text_is_related(text, text, text) from public, anon, authenticated;
