-- The lab section that owns a contract (e.g. "งานอณูชีววิทยา" for a Viral Load
-- analyzer lease) already lives on public.contracts as `department`, carried
-- over from the legacy portal's own contract form. LABCBH Stock never read or
-- wrote it, so every contract created or edited here since cutover has left
-- the column untouched. This migration adds the same guard the legacy portal
-- enforced with a UI dropdown, and teaches create_contract/update_contract to
-- read and write it, so it stops silently going stale.
--
-- Deliberately NOT NULL-free: the legacy_import RPC in
-- 20260729192247_lab_stock_import_staging.sql inserts contracts without a
-- department, and a future re-run of that one-time import must not start
-- failing a NOT NULL constraint it was never written against. A CHECK is
-- enough to keep every department value in this list; NULL still satisfies a
-- Postgres CHECK, same as contracts_contract_type_check already relies on for
-- contract_type.
begin;
...
begin
  if not exists (
    ...
    where conrelid = 'public.contracts'::regclass
      and conname = 'contracts_department_check'
  ) then
...
-- Reproduced verbatim from 20260730121000_lab_stock_lease_without_items.sql
-- apart from department handling.
