...
assert.match(fields, /showCommitteeValidationErrors/, 'committee validation remains available as an internal guard while its create-page copy can be hidden')
...
assert.match(form, /showCommitteeValidationErrors=\{isEditMode\}/, 'new PR creation must hide the repeated committee-count messages')
...
assert.match(detail, /export function PurchaseRequestChecklistActions/, 'stock officer checklist actions should be independently placeable in the panel header')
assert.match(detail, /pr-stock-officer__actions/, 'checklist actions should use the stock officer header placement')
assert.match(detail, /dialogRef\.current\?\.showModal\(\)/, 'committee PDF should open from the header action')
assert.match(detail, /isCommitteePdfOpen &&/, 'committee PDF should load only after the user opens the preview')
...
assert.doesNotMatch(detail, /<h3[^>]*>เอกสารแนบ<\/h3>/, 'the detail view should not repeat a combined attachment count heading')
assert.doesNotMatch(detail, /<span>\{checklist\.committees\.length\} รายชื่อ<\/span>/, 'the committee heading should not repeat a combined member count')
...
assert.match(detailPage, /PurchaseRequestChecklistActions/, 'stock officer actions belong in the stock officer header')
assert.match(detailPage, /pr-stock-officer__header[\s\S]*PurchaseRequestChecklistActions/, 'header actions should render beside the stock officer title')
...
assert.doesNotMatch(contractRoster, /รายชื่อกรรมการประจำสัญญา/, 'contract roster must not repeat its Thai title')
assert.doesNotMatch(contractRoster, /PR ที่ซื้อในสัญญาจะดึงรายชื่อนี้ไปอัตโนมัติ/, 'contract roster must not show the automatic-copy helper text')
assert.doesNotMatch(contractRoster, /สัญญาเดิมนี้ยังไม่มี roster กรรมการ/, 'contract roster must not show the legacy empty-state copy')
...
assert.match(styles, /\.pr-checklist__section \{[^}]*padding: 24px 22px var\(--space-6\);/, 'checklist sections need bottom breathing room between nested and outer borders')
assert.match(styles, /\.pr-checklist__committees fieldset \{[^}]*padding: var\(--space-6\);/, 'committee cards need a full 24px inset so nested borders do not feel cramped')
assert.match(styles, /\.pr-checklist__committee-grid \{[^}]*gap: var\(--space-5\);/, 'committee columns need a 20px rhythm between adjacent controls')
...
assert.match(styles, /@media \(max-width: 800px\) \{[\s\S]*?\.pr-stock-officer__actions/, 'stock officer header actions must remain usable on tablet/mobile widths')
