...
const lotMigrationNames = readdirSync(migrationsDir).filter((name) => name.endsWith('_inventory_lot_stock_checklist.sql'))
assert.equal(lotMigrationNames.length, 1, 'the lot-level weekly stock-check migration must exist exactly once')
const lotMigrationSql = read(join(migrationsDir, lotMigrationNames[0]))
...
assert.match(lotMigrationSql, /create table if not exists public\.inventory_lot_stock_checks/i)
assert.match(lotMigrationSql, /inventory_lot_id uuid not null references public\.inventory_lots\(id\)/i)
assert.match(lotMigrationSql, /inventory_lot_stock_checks_lot_checked_at_idx/i)
assert.match(lotMigrationSql, /create or replace view public\.inventory_lot_latest_stock_checks/i)
assert.match(lotMigrationSql, /create or replace view public\.inventory_item_latest_stock_checks/i)
assert.match(lotMigrationSql, /all active lots with a positive balance were checked this week/i)
assert.match(lotMigrationSql, /create or replace function public\.record_inventory_lot_stock_check\([\s\S]*?p_actor_id uuid[\s\S]*?\)/i)
assert.match(lotMigrationSql, /inventory lot does not belong to inventory item/i)
assert.match(lotMigrationSql, /inventory lot has no stock to check/i)
assert.match(lotMigrationSql, /revoke execute on function public\.record_inventory_lot_stock_check\(uuid, uuid, uuid\) from authenticated/i)
assert.match(lotMigrationSql, /grant execute on function public\.record_inventory_lot_stock_check\(uuid, uuid, uuid\) to service_role/i)
...
assert.match(queries, /export async function listInventoryChecklistItems/)
assert.match(queries, /inventory_lot_latest_stock_checks/)
assert.match(queries, /checklistLots/)
...
assert.match(types, /export interface InventoryLotStockCheckResult/)
assert.match(types, /export interface InventoryChecklistLotRecord/)
assert.match(types, /export interface InventoryChecklistItemRecord/)
...
assert.match(actions, /export async function recordInventoryLotStockCheck/)
assert.match(actions, /supabaseAdmin\.rpc\('record_inventory_lot_stock_check'/)
...
assert.match(checklistPage, /listInventoryChecklistItems/)
assert.match(checklistPage, /checklistLots\.every/)
...
assert.match(checklistTable, /recordInventoryLotStockCheck/)
assert.match(checklistTable, /InventoryChecklistLotRecord/)
assert.match(checklistTable, /checklistLots\.every/)
assert.match(checklistTable, /LOT \{lot\.lotNumber\}/)
...
assert.match(checklistTable, /ตรวจครบทุก lot แล้ว/)
assert.match(checklistTable, /ตรวจ lot แล้ว/)
...
assert.doesNotMatch(adjustmentDialog, /ต้องระบุเลขล็อตและวันหมดอายุทุกครั้ง ระบบจะบันทึกเฉพาะส่วนต่างลงบัญชีตรวจสอบย้อนหลัง/)
assert.match(adjustmentDialog, /stock-adjustment-dialog__form/)
assert.match(adjustmentDialog, /stock-adjustment-dialog__actions/)
