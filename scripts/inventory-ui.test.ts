...
assert.match(createActions, /getInventoryItemSummary/, 'the catalogue summary must load through a server action')
...
assert.match(detailPage, /searchParams:\s*Promise</, 'ledger pagination must read the URL state on the server')
...
assert.match(table, /InventoryItemSummaryDialog/, 'the catalog must open an item summary popup')
assert.match(table, /<InventoryItemSummaryDialog item=\{item\} \/>/, 'the reagent name must open the compact summary popup')
assert.match(table, /href=\{`\/inventory\/\$\{item\.id\}`\}/, 'the action-bar document icon must keep linking to full detail')
...
assert.match(summaryDialog, /useDeferredDialog\(\)/, 'the item detail action must open a deferred native dialog')
assert.match(summaryDialog, /getInventoryItemSummary/, 'the popup must load the latest item details on demand')
assert.match(summaryDialog, /LOT_EXPIRY_LABELS/, 'every lot must show a written expiry status')
assert.match(summaryDialog, /formatThaiDate\(lot\.expiryDate\)/, 'every lot must show its expiry date')
assert.match(summaryDialog, /formatQuantity\(lot\.balance/, 'every lot must show its current balance')
assert.match(summaryDialog, /!lot\.isActive/, 'the compact popup must replace every expiry badge for inactive lots')
assert.match(summaryDialog, /!summary\.isActive \? \(/, 'the compact popup must replace the stock-level badge for inactive items')
assert.match(summaryDialog, /DetailIconLink/, 'the popup must retain a path to the full item detail')
...
assert.match(lotTable, /InventoryLotActiveControl/, 'stock operators need a per-lot active-state control')
assert.match(lotTable, /lot\.isActive/, 'the lot table must show inactive lots explicitly')
assert.match(lotTable, /!lot\.isActive/, 'inactive lots must replace every expiry badge')
...
assert.match(presenter, /INACTIVE_STATUS_TONE = 'neutral'/, 'inactive status must use a distinct neutral tone')
...
assert.match(actions, /supabaseAdmin\.rpc\('set_inventory_lot_active'/, 'lot active state must be written through a server RPC')
...
assert.match(queries, /INVENTORY_MOVEMENT_PREVIEW_SIZE = 5/, 'the ledger preview must be five movements')
assert.match(queries, /INVENTORY_MOVEMENT_PAGE_SIZE = 20/, 'the expanded ledger must use twenty movements per page')
assert.match(queries, /count: 'exact'/, 'the ledger must know the total count for pagination')
assert.match(queries, /\.range\(movementFrom, movementFrom \+ movementPageSize - 1\)/, 'the ledger query must fetch one page at a time')
