...
assert.match(expenseForm, /expense-entry--contract/, 'contract expense entry must identify its scoped visual treatment')
...
assert.match(globalStyles, /\.expense-entry--contract \.expense-entry__glyph\s*\{[\s\S]*color:\s*var\(--lab-surface\);[\s\S]*background:\s*var\(--lab-primary\);/, 'the contract expense trigger icon must use a high-contrast primary tile')
