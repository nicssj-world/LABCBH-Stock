...
assert.match(detailPage, /contract\.expiryNotice[\s\S]*contract-expiry-notice/, 'an imminent contract expiry must appear as a dedicated readable notice in the detail header')
assert.match(detailPage, /role="status"/, 'contract expiry notices must announce their status without relying on color alone')
...
assert.match(globalStyles, /\.contract-expiry-notice--danger/, 'the final-30-day deadline needs a distinct urgent visual treatment')
