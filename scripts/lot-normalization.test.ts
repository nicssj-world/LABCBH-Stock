...
const compatibilityNames = readdirSync(migrationsDir).filter((name) => name.endsWith('_inventory_lot_key_compatibility.sql'))
assert.equal(compatibilityNames.length, 1, 'exactly one lot-key compatibility migration must exist')
const compatibilitySql = readFileSync(join(migrationsDir, compatibilityNames[0]), 'utf8')
...
assert.match(compatibilitySql, /generated always as \(upper\(btrim\(lot_number\)\)\) stored/i)
assert.match(compatibilitySql, /inventory_lots_item_lot_key/i)
assert.doesNotMatch(compatibilitySql, /create or replace function public\.post_goods_receipt/i)
