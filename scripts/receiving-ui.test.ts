...
assert.match(listPage, /showCancelled/, 'cancelled receipts must have an explicit visibility toggle')
assert.match(listPage, /receipt\.status !== 'cancelled'/, 'cancelled receipts must be hidden from the default register')
assert.match(listPage, /แสดง(?:รายการ|ใบ)ยกเลิก/, 'the register must provide a button to show cancelled receipts')
assert.match(listPage, /ซ่อน(?:รายการ|ใบ)ยกเลิก/, 'the cancelled visibility button must be reversible')
...
assert.match(listPage, /receipt-table--desktop/, 'the wide receiving register must have an explicit desktop-only presentation')
assert.match(listPage, /className="receipt-task-cards"/, 'the receiving register must provide task cards on phones')
assert.match(listPage, /GoodsReceiptSummaryDialog receipt=\{receipt\} variant="card"/, 'phone receiving cards must keep the same compact summary action')
...
assert.match(form, /receipt-balance-table--desktop/, 'the PR balance table must have a desktop-only presentation')
assert.match(form, /className="receipt-balance-cards"/, 'the selected PR balance must provide phone-friendly cards')
