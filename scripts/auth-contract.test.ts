...
assert.match(login, /router\.replace\('\/dashboard'\)/, 'successful login must navigate to the dashboard')
assert.doesNotMatch(login, /router\.refresh\(\)/, 'successful login must not trigger a duplicate dashboard render')
