...
const queueMigration = read('supabase/migrations/20260821190000_lab_stock_notification_queue.sql')
...
assert.match(queueMigration, /add column if not exists dismissed_at timestamptz/)
assert.match(queueMigration, /update public\.lab_stock_notifications notification/)
assert.match(queueMigration, /request\.status <> 'pending'/)
assert.match(queueMigration, /requisition\.status <> 'waiting'/)
assert.match(queueMigration, /where resolved_at is null and dismissed_at is null/)
assert.match(queueMigration, /create or replace function public\.mark_all_lab_stock_notifications_read/)
assert.match(queueMigration, /set read_at = coalesce\(read_at, now\(\)\)/)
assert.match(queueMigration, /dismissed_at = coalesce\(dismissed_at, now\(\)\)/)
assert.match(queueMigration, /where recipient_id = p_actor_id\s*\n\s*and resolved_at is null\s*\n\s*and dismissed_at is null/)
assert.doesNotMatch(queueMigration, /security definer/i, 'notification queue functions must not expose a public security-definer function')

assert.match(queries, /canOperateStock\(actor\)/)
assert.match(queries, /\.eq\('status', 'pending'\)/)
assert.match(queries, /\.eq\('status', 'waiting'\)/)
assert.match(queries, /servicePurchaseRequests/)
assert.match(queries, /pendingServicePurchaseRequests: servicePurchaseRequests\.count \?\? 0/)
assert.match(queries, /\.eq\('recipient_id', actor\.id\)/)
assert.match(queries, /\.is\('read_at', null\)/)
assert.match(queries, /\.is\('resolved_at', null\)/)
assert.match(queries, /\.is\('dismissed_at', null\)/)
assert.match(queries, /dismissed_at/)
...
assert.match(shell, /pendingServicePurchaseRequests/)
assert.match(shell, /waitingRequisitions/)
assert.match(types, /pendingServicePurchaseRequests: number/)
assert.match(center, /postgres_changes/)
assert.match(center, /event: 'INSERT'/)
assert.match(center, /event: 'UPDATE'/)
assert.match(center, /auth\.getSession\(\)/, 'Realtime must wait for the browser session before subscribing')
assert.match(center, /auth\.onAuthStateChange/, 'Realtime must reconnect when the browser session becomes available')
assert.match(center, /realtime\.setAuth\(accessToken\)/, 'Realtime must receive the authenticated access token')
assert.match(center, /if \(!accessToken\)/, 'Realtime must not subscribe as anon')
assert.match(center, /nextChannel\.subscribe\(\)/, 'Realtime should subscribe only after the authenticated channel is built')
assert.match(center, /dismissed_at/)
assert.match(center, /notification\.resolvedAt \|\| notification\.dismissedAt/)
assert.match(center, /pendingServicePurchaseRequests: notification\.eventType === 'service_purchase_request_created'/)
assert.match(center, /notifications: \[\]/)
