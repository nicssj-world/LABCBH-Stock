...
import { assertServiceStockOperator } from './authorization'
import { buildServicePurchaseRequestLineFlexMessage } from './line-notification'
...
async function completeAttempt({
  ...
  const result = await supabaseAdmin.rpc('complete_service_purchase_request_line_notification', {
...
export async function notifyServicePurchaseRequestInLine(
  ...
  const documentUrl = `${config.appBaseUrl}/service-procurement/purchase-requests/${parsedRequestId}`
  const beginResult = await supabaseAdmin.rpc('begin_service_purchase_request_line_notification', {
  ...
  if (beginResult.error) throw actionError('เริ่มแจ้งใบ PR งานจ้างใน LINE', beginResult.error.message)
  ...
  try {
    const sent = await pushLineFlexMessage({
      ...
      message: buildServicePurchaseRequestLineFlexMessage(attempt),
    })
    ...
    revalidatePath(`/service-procurement/purchase-requests/${parsedRequestId}`)
    return { id: completion.id, status: completion.status, completedAt: completion.completed_at, poNumber: completion.po_number, errorMessage: completion.error_message }
  } catch (caught) {
    const error = caught instanceof LineApiError ? caught : new LineApiError('เกิดข้อผิดพลาดระหว่างส่งข้อความ LINE', 'unknown')
    ...
    revalidatePath(`/service-procurement/purchase-requests/${parsedRequestId}`)
    if (completionError) throw actionError('แจ้งใบ PR งานจ้างใน LINE', `${error.message} และบันทึกประวัติไม่สำเร็จ: ${completionError}`)
    if (error.outcome === 'unknown') throw actionError('แจ้งใบ PR งานจ้างใน LINE', `${error.message} กรุณาตรวจสอบกลุ่มก่อนส่งซ้ำ`)
    throw actionError('แจ้งใบ PR งานจ้างใน LINE', error.message)
  }
}
