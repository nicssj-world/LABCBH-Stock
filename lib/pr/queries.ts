...
const requestRowSchema = z.object({
  ...
  acknowledged_by_name: z.string().nullable().default(null),
  ...
  acknowledger: z.object({ name: z.string().nullable(), ephis_id: z.string().nullable() }).nullable(),
...
const REQUEST_SELECT = `
  ...
  acknowledger:profiles!purchase_requests_acknowledged_by_fkey (name, ephis_id),
...
function mapRequest(row: z.infer<typeof requestRowSchema>): PurchaseRequestRecord {
  ...
  return {
    ...
    acknowledgedByEphisId: row.acknowledger?.ephis_id?.trim() || null,
    // Prefer the immutable snapshot so every authorised reader sees the
    // officer who confirmed the PR even when the profile relationship is
    // hidden by RLS.
    acknowledgedByName: row.acknowledged_by_name?.trim() || row.acknowledger?.name?.trim() || null,
