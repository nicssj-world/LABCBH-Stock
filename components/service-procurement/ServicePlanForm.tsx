...
export function ServicePlanForm({ mode, departments, initial, defaultFiscalYear, hasRequests = false }: Props) {
  ...
  return (
    <form className="route-stack service-plan-form" onSubmit={submit}>
      ...
      {isRedCross && <section className="bench-panel" aria-labelledby="service-plan-test-items-title"><div className="bench-panel__header"><div><p className="section-kicker">RED CROSS TEST ITEMS</p><h2 id="service-plan-test-items-title">รายการส่งตรวจ</h2></div><Button type="button" variant="secondary" onClick={() => setItems((current) => [...current, { name: '', unit: '', unitPrice: '' }])} disabled={pending}>เพิ่มรายการ</Button></div>{items.length === 0 ? <p className="empty-state">ยังไม่มีรายการส่งตรวจ (สามารถเพิ่มภายหลังได้)</p> : <div className="service-ledger-table-wrap"><table className="data-table service-plan-test-items-table"><thead><tr><th>ชื่อรายการ</th><th>หน่วย</th><th className="numeric-cell">ราคาต่อหน่วย (บาท)</th><th /></tr></thead><tbody>{items.map((item, index) => <tr key={`${index}-${item.name}`}><td><input required maxLength={240} aria-label={`ชื่อรายการส่งตรวจที่ ${index + 1}`} value={item.name} onChange={(event) => updateItem(index, 'name', event.target.value)} /></td><td><input required maxLength={100} aria-label={`หน่วยรายการส่งตรวจที่ ${index + 1}`} value={item.unit} onChange={(event) => updateItem(index, 'unit', event.target.value)} /></td><td><input required type="number" min="0.01" step="0.01" inputMode="decimal" aria-label={`ราคาต่อหน่วยรายการส่งตรวจที่ ${index + 1}`} value={item.unitPrice} onChange={(event) => updateItem(index, 'unitPrice', event.target.value)} /></td><td><Button type="button" variant="ghost" onClick={() => setItems((current) => current.filter((_, itemIndex) => itemIndex !== index))}>นำออก</Button></td></tr>)}</tbody></table></div>}</section>}
