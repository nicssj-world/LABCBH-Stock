  ...
  return (
    <form className="route-stack service-plan-form" onSubmit={submit}>
      <section className="bench-panel"><div className="bench-panel__header"><div><p className="section-kicker">PLAN DETAILS</p><h2>ข้อมูลแผนงานจ้าง</h2></div></div><div className="form-grid">
        ...
        <label className="field-row"><span>ปีงบประมาณ <span className="field-required" aria-hidden="true">*</span></span>{mode === 'edit' ? <><input required type="number" value={fiscalYear} readOnly aria-readonly="true" /><small className="field-help">ปีงบประมาณของแผนที่สร้างแล้วไม่สามารถแก้ไขได้</small></> : <input required type="number" min="2500" max="3000" value={fiscalYear} onChange={(event) => setFiscalYear(event.target.value)} />}</label>
      </div></section>
