...
interface OutLabFiltersProps {
  ...
  cadences: Array<{ value: string; label: string }>
  departments: string[]
}

export function OutLabFilters({
  ...
  // showEnded and showOlder are visibility controls, not filters. Clearing the
  // filters must not silently hide rows the reader chose to see.
  const clearFilters = () => {
    setSearch('')
  ...
  return (
    <form className="filter-bench contract-filters" onSubmit={handleSubmit} aria-label="ตัวกรองสัญญา Out Lab">
      ...
      <label>
        รูปแบบงบ
        <select name="kind" value={kind} onChange={handleSelectChange}>
          <option value="">ทุกรูปแบบ</option>
          {kinds.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}
        </select>
      </label>
      <label>
        จังหวะลงข้อมูล
      ...
      <p className="contract-filters__status" aria-live="polite">
        {isPending ? 'กำลังกรองรายการ…' : 'เลือกตัวกรองเพื่อแสดงผลทันที'}
      </p>
    </form>
  )
}
