...
import { outLabFileUrl, removeOutLabFile } from '@/lib/out-lab/file-actions'
...
function uploadWithProgress(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    xhr.open('POST', `/api/out-lab/${contractId}/file`)
...
export interface OutLabFileCardProps {
  contractId: string
  filePath: string | null
  canEdit: boolean
}

export function OutLabFileCard({ contractId, filePath, canEdit }: OutLabFileCardProps) {
  ...
  const open = () => {
    ...
    startTransition(async () => {
      try {
        const signedUrl = await outLabFileUrl(contractId, filePath)
  ...
  const detach = () => {
    ...
    startTransition(async () => {
      try {
        await removeOutLabFile(contractId)
        closePreview()
        router.refresh()
      } catch (caught) {
  ...
  return (
    <div className="contract-file-card" data-has-file={filePath ? 'true' : 'false'}>
      ...
      {/* Opened in-page rather than in a new tab: the signed URL is short-lived
          and a stray tab would outlive it. */}
      <dialog
        ref={previewDialogRef}
        className="app-dialog file-preview-dialog"
        aria-labelledby="out-lab-file-preview-title"
        onCancel={(event) => {
      ...
      >
        <header className="app-dialog__header">
          <div>
            <h2 id="out-lab-file-preview-title">ไฟล์สัญญา</h2>
