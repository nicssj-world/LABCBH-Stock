...
import { listServiceCommitteeCandidates, listServicePlans } from '@/lib/service-procurement/queries'
...
export default async function NewServicePurchaseRequestPage() {
  ...
  const [plans, candidates] = await Promise.all([listServicePlans(), listServiceCommitteeCandidates()])
  ...
  return <div className="route-stack"><header className="page-heading"><div><Link className="back-link" href="/service-procurement/purchase-requests">← ใบ PR (งานจ้าง)</Link><p className="section-kicker">NEW SERVICE PURCHASE REQUEST</p><h1>สร้างใบ PR (งานจ้าง)</h1><p>ระบบจะสำรองวงเงินเต็มจำนวนเมื่อส่ง PR และตัดยอดใช้จริงเมื่อผู้มีสิทธิ์ปิด PO</p></div></header><ServicePurchaseRequestForm department={requesterDepartment} departments={departments} requesterName={actor.name?.trim() || (actor.ephisId ? `E-Phis ${actor.ephisId}` : actor.id)} plans={plans} candidates={candidates} /></div>
}
