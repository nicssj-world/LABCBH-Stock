...
export default async function ServicePlanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="route-stack service-plan-detail-page">
    ...
    <section className="bench-panel service-plan-documents-panel"><div className="bench-panel__header"><div><p className="section-kicker">PLAN DOCUMENTS</p><h2>เอกสารระดับแผน</h2></div><p>เก็บจนกว่าจะปิดปีงบประมาณ</p></div>{plan.documents.length === 0 ? <p className="empty-state">ยังไม่มีใบเสนอราคาหรือหน้าสัญญา</p> : <ul className="service-document-list">{plan.documents.map((document) => <li key={document.id}><span>{document.kind === 'quotation' ? 'ใบเสนอราคา' : 'หน้าสัญญา'}</span><a href={`/api/service-procurement/plans/${plan.id}/documents/${document.id}`} target="_blank" rel="noreferrer">{document.fileName}</a></li>)}</ul>}</section>
